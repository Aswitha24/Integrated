...
export const createPayment=(payment)=>async dispatch=> {
    try {
        await axios.post ("http://localhost:8081/api/v2/Payment",payment);
...
export const getPayments=()=>async dispatch=>{
    const res=await axios.get("http://localhost:8081/api/v2/allPayments");
    dispatch({
        type:GET_PAYMENTS,
        payload:res.data
    })
...
export const getPayment=(paymentId,history)=>async dispatch=>{
    const res=await axios.get(`http://localhost:8081/api/v2/Payment/${paymentId}`);
    dispatch({
        type:GET_PAYMENT,
        payload:res.data
    })
...
export const deletePayment=(paymentId)=>async dispatch=>{
    if(window.confirm("Are you sure ? This will delete the project and the data related to it")) {
        await axios.delete(`http://localhost:8081/api/v2/Payment/${paymentId}`);
        dispatch({
           type:DELETE_PAYMENT,
            payload:paymentId
        })
